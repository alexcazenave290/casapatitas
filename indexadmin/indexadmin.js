...
window.addEventListener('load', function() {
  ...
  if (loadingScreen) {
    // Esperar a que termine la animación (1.5s) antes de ocultar
    setTimeout(function() {
...
async function checkSessionAndToggleUI() {
  ...
  try {
    ...
    isUserLogged = false;
    console.error('[ADMIN] Error al verificar sesión:', e);
    if (loginBtn)    loginBtn.style.display = "block";
    if (profileIcon) profileIcon.style.display = "none";
  }
}
        ...
        if (data.success) {
          ...
          if (rol === 'institucion') {
            camposFormulario = `
              ...
              <input type="text" name="telefono" placeholder="Teléfono" value="${p.telefono ?? ''}" maxlength="8" style="padding:8px 12px;border-radius:8px;border:1px solid #ccc;width:100%;">
            ...
            camposFormulario = `
              ...
              <input type="text" name="cedula" placeholder="Cédula" value="${p.cedula ?? ''}" maxlength="8" style="padding:8px 12px;border-radius:8px;border:1px solid #ccc;width:100%;">
              <input type="text" name="telefono" placeholder="Teléfono" value="${p.telefono ?? ''}" maxlength="8" style="padding:8px 12px;border-radius:8px;border:1px solid #ccc;width:100%;">
              <input type="text" name="direccion" placeholder="Dirección" value="${p.direccion ?? ''}" style="padding:8px 12px;border-radius:8px;border:1px solid #ccc;width:100%;">
            `;
          }
          ...
          document.getElementById("editar-perfil-form").onsubmit = async (e) => {
            ...
            if (payload.telefono && payload.telefono.trim() !== '') {
              if (!/^\d{8}$/.test(payload.telefono)) {
                alert("❌ El teléfono debe tener exactamente 8 dígitos numéricos. Eso no es un número telefónico.");
                return;
              }
            }
...
const razasPorEspecie = {
  'Perro': ['Golden Retriever', 'Bulldog', 'Labrador', 'Caniche', 'Pastor Alemán', 'Chihuahua', 'Pitbull', 'Beagle', 'Husky Siberiano', 'Sin Raza'],
  'Gato': ['Siamés', 'Persa', 'Egipcio', 'Maine Coon', 'Bengalí', 'Británico de Pelo Corto', 'Angora', 'Ragdoll', 'Sin Raza'],
  'Ave': ['Canario', 'Periquito', 'Loro', 'Cacatúa', 'Guacamayo', 'Agapornis', 'Ninfa', 'Sin Raza'],
...
let mensajesCache = []; // Cache de mensajes para evitar re-render innecesario
let esPrimeraCargaChat = false; // Flag para indicar si es la primera carga del chat
...
async function iniciarChat(emailDestinatario, tipoDestinatario, nombreDestinatario, logoDestinatario) {
  try {
    ...
    if (data.success) {
      ...
      // Abrir modal de chat
      esPrimeraCargaChat = true; // Marcar como primera carga
      abrirModalChat();
      ...
      chatPollingInterval = setInterval(cargarMensajes, 2000);
...
function cerrarModalChat() {
  ...
  // Limpiar variables del chat
  currentConversacionId = null;
  currentChatUser = null;
  mensajesCache = []; // Limpiar cache de mensajes
  esPrimeraCargaChat = false; // Resetear flag
}
...
function renderizarMensajes(mensajes) {
  ...
  // Comparar mensajes con cache para detectar cambios
  const mensajesIguales = JSON.stringify(mensajesCache) === JSON.stringify(mensajes);
  
  // Si no hay cambios y no es la primera carga, no hacer nada
  if (mensajesIguales && !esPrimeraCargaChat) {
    return;
  }
  
  // Si es la primera carga o hay cambios significativos, hacer render completo
  if (esPrimeraCargaChat || mensajesCache.length === 0) {
    esPrimeraCargaChat = false; // Resetear flag
  }
  ...
  // Actualizar cache
  mensajesCache = JSON.parse(JSON.stringify(mensajes));
...
async function cargarNotificaciones() {
  ...
  try {
    ...
    if (data.success) {
      console.log('[ADMIN] Actualizando badge con', data.no_leidas, 'notificaciones no leídas');
      actualizarBadgeNotificaciones(data.no_leidas);
      return data;
    } else {
      console.error('[ADMIN] API retornó error:', data);
    }
  } catch (error) {
...
function iniciarPollingNotificaciones() {
  ...
  if (isUserLogged) {
    ...
    notificacionesPollingInterval = setInterval(() => {
      console.log('[ADMIN] Polling de notificaciones ejecutándose...');
      cargarNotificaciones();
    }, 30000);
