...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (formUsuario) {
    formUsuario.onsubmit = async function(e) {
      ...
      // Validar teléfono (8 dígitos numéricos)
      ...
      formData.append("ci_us", cedula);
      formData.append("tel_us", telefono);
      ...
      try {
        ...
        if (data.success) {
          ...
          // Redirigir según el rol (Usuario va a index.html)
          if (data.rol === "usuario") {
  ...
  if (formEmpleado) {
    formEmpleado.onsubmit = async function(e) {
      ...
      // Validar cédula (8 dígitos numéricos)
      const cedula = document.getElementById("cedula_empleado").value;
      if (!/^\d{8}$/.test(cedula)) {
        alert("❌ La cédula debe tener exactamente 8 dígitos numéricos.");
        return;
      }
      ...
      formData.append("cedula_empl", cedula);
      formData.append("tel_empl", telefono);
      ...
      try {
        ...
        if (data.success) {
          ...
          // Redirigir según el rol (Empleado va a indexadmin.html)
  ...
  if (formInstitucion) {
    formInstitucion.onsubmit = async function(e) {
      ...
      // Validar teléfono (8 dígitos numéricos)
      const telefono = document.getElementById("tel_inst").value;
      if (!/^\d{8}$/.test(telefono)) {
        alert("❌ El teléfono debe tener exactamente 8 dígitos numéricos. Eso no es un número telefónico.");
        return;
      }
      ...
      formData.append("tel_inst", telefono);
      ...
      try {
        ...
        if (data.success) {
          ...
          // Redirigir según el rol (Institución va a indexadmin.html)
