...
try {
  ...
  if ($tipo === 'usuario') {
    ...
    // Obtener cédula y teléfono del POST
    $ci_us = trim($_POST['ci_us'] ?? '');
    $tel_us = trim($_POST['tel_us'] ?? '');

    $ins = $pdo->prepare("INSERT INTO Usuario (mail_us, contrasena_us, ci_us, nom_us, apell_us, tel_us, direccion_us, adoptante, donante)
                          VALUES (?, ?, ?, ?, NULL, ?, NULL, DEFAULT, DEFAULT)");
    $ins->execute([$mail, $hash, $ci_us, $user, $tel_us]);
  ...
  elseif ($tipo === 'institucion') {
    ...
    // Crear sesión automáticamente después del registro
    session_regenerate_id(true);
    $_SESSION['user_mail'] = $email_inst;
    $_SESSION['user_name'] = $nomb_inst;
    $_SESSION['user_rol'] = 'institucion';
  }
  ...
  // Devolver el rol para redirección automática
