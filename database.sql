...
CREATE TABLE Donacion (
  ...
  donacion_pagina BOOLEAN NOT NULL DEFAULT FALSE,
  CONSTRAINT FK_donacion_inst FOREIGN KEY (id_inst) REFERENCES Institucion(id_inst)
);
...
-- logo_inst ya está en el CREATE TABLE de Institucion
...
-- ===== SISTEMA DE ADOPCIÓN MEDIANTE CHAT =====
-- Agregar columna para vincular mensajes con solicitudes de adopción
ALTER TABLE mensajes 
ADD COLUMN id_mascota_adopcion INT NULL,
ADD CONSTRAINT fk_mensaje_mascota 
FOREIGN KEY (id_mascota_adopcion) REFERENCES Mascota(id_masc) 
ON DELETE CASCADE;

-- Crear índice para mejorar el rendimiento de las consultas de adopción
CREATE INDEX idx_mensaje_mascota ON mensajes(id_mascota_adopcion);

-- ===== SISTEMA DE COMENTARIOS EN MASCOTAS =====
CREATE TABLE Comentarios (
  ...
  -- Foreign keys
  CONSTRAINT FK_comentario_masc FOREIGN KEY (id_masc) REFERENCES Mascota(id_masc) ON DELETE CASCADE,
  CONSTRAINT FK_comentario_usuario FOREIGN KEY (mail_us) REFERENCES Usuario(mail_us) ON DELETE CASCADE,
  CONSTRAINT FK_comentario_empleado FOREIGN KEY (mail_empl) REFERENCES Empleado(mail_empl) ON DELETE CASCADE,
  CONSTRAINT FK_comentario_institucion FOREIGN KEY (email_inst) REFERENCES Institucion(email_inst) ON DELETE CASCADE,
  
  -- Constraint para asegurar que solo uno de los 3 campos esté lleno
  CONSTRAINT CHK_comentario_user_type CHECK (
    (mail_us IS NOT NULL AND mail_empl IS NULL AND email_inst IS NULL) OR 
    (mail_us IS NULL AND mail_empl IS NOT NULL AND email_inst IS NULL) OR 
    (mail_us IS NULL AND mail_empl IS NULL AND email_inst IS NOT NULL)
  ),
  
  -- Índice para optimizar búsquedas por mascota
  INDEX idx_comentario_mascota (id_masc, fecha_comentario DESC)
);
...
CREATE TABLE Seguimiento (
  ...
  comportamiento TEXT,
  observaciones TEXT,
  foto_seguimiento VARCHAR(200) NULL,
  fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  
  -- Foreign keys
  CONSTRAINT FK_seguimiento_adop FOREIGN KEY (id_adop) REFERENCES Adopcion(id_adop) ON DELETE CASCADE,
  CONSTRAINT FK_seguimiento_masc FOREIGN KEY (id_masc) REFERENCES Mascota(id_masc) ON DELETE CASCADE,
  CONSTRAINT FK_seguimiento_us FOREIGN KEY (mail_us) REFERENCES Usuario(mail_us) ON DELETE CASCADE,
  
  -- Índice para optimizar búsquedas
  INDEX idx_seguimiento_usuario (mail_us, fecha_seguimiento DESC),
  INDEX idx_seguimiento_mascota (id_masc, fecha_seguimiento DESC)
);
...
CREATE TABLE notificaciones (
    ...
    tipo_notificacion ENUM('mensaje', 'seguimiento', 'donacion', 'comentario') NOT NULL,
...
-- ===== MODIFICACIONES PARA SISTEMA DE DONACIONES =====
-- Agregar campos para registrar donaciones con más detalle
ALTER TABLE Donacion 
MODIFY COLUMN id_Don INT AUTO_INCREMENT,
ADD COLUMN email_institucion VARCHAR(100) DEFAULT NULL AFTER id_inst,
ADD COLUMN plataforma VARCHAR(50) DEFAULT NULL AFTER medioPago_don,
ADD COLUMN tipo_donacion VARCHAR(50) DEFAULT 'general' AFTER destino_don,
ADD COLUMN nombre_refugio VARCHAR(100) DEFAULT NULL AFTER tipo_donacion,
ADD INDEX idx_institucion (email_institucion),
ADD INDEX idx_fecha (fecha_don DESC);

-- Agregar foreign key para la institución destinataria
ALTER TABLE Donacion 
ADD CONSTRAINT FK_donacion_inst_dest 
FOREIGN KEY (email_institucion) 
REFERENCES Institucion(email_inst) 
ON DELETE SET NULL;
